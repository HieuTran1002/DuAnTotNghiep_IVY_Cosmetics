...
//    @Query("SELECT DISTINCT new com.example.duantotnghiep.response.GiamGiaResponse(gg.id, gg.ma, gg.ten, gg.ngayBatDau, gg.ngayKetThuc, gg.hinhThucGiam, gg.trangThai, spgg.mucGiam) " +
//            "FROM GiamGia gg " +
//            "LEFT JOIN gg.spGiamGiaList spgg " +
//            "LEFT JOIN spgg.sanPham sp " +
//            "WHERE (:ma is null or gg.ma LIKE %:ma%) " +
//            "AND (:ten is null or gg.ten LIKE %:ten%) " +
...
//            Pageable pageable);
    ...
    @Query("SELECT  new com.example.duantotnghiep.response.GiamGiaResponse(gg.id, gg.ma,gg.ten, gg.ngayBatDau, gg.ngayKetThuc, gg.hinhThucGiam, gg.trangThai, spgg.mucGiam,sp.ten,sp.giaBan,spgg.donGiaKhiGiam,sp.id) "
            +
            "FROM GiamGia gg " +
            "JOIN gg.spGiamGiaList spgg " +
            "JOIN spgg.sanPham sp " +
    ...
    @Query("SELECT new com.example.duantotnghiep.response.GiamGiaResponse(gg.id,gg.ma,gg.ten,gg.ngayBatDau,gg.ngayKetThuc,gg.hinhThucGiam,gg.trangThai,spgg.mucGiam) "
            +
            "FROM GiamGia gg " +
            "JOIN gg.spGiamGiaList spgg " +
            " JOIN spgg.sanPham sp " +
            "WHERE gg.ma LIKE :key OR gg.ten LIKE :key ")
    ...
    @Query("SELECT sp.id, i.tenImage, sp.ten, sp.giaBan FROM SanPham sp JOIN sp.listImage i WHERE i.isDefault = TRUE AND sp.ten LIKE :key ")
    List<Object[]> ProductDetailResponse(@Param("key") String key);

//        @Query("SELECT sp.id, i.tenImage as ten, sp.ten, sp.giaBan FROM SanPham sp JOIN sp.listImage i WHERE i.isDefault = TRUE AND (:ten is null or sp.ten LIKE %:ten%) ")

    @Query("SELECT sp.id, i.tenImage, sp.ten, sp.giaBan FROM SanPham sp " +
    ...
    @Query("SELECT sp.id, sp.ten, sp.giaBan " +
            "FROM SanPham sp " +
            "JOIN sp.thuongHieu th " +
            "JOIN sp.danhMuc dm " +
            "JOIN sp.xuatXu xx " +
...
//        @Query("SELECT sp.id, i.tenImage , sp.ten, sp.giaBan FROM SanPham sp LEFT JOIN sp.listImage i WHERE i.isDefault = TRUE AND (:ten is null or sp.ten LIKE %:ten%) AND sp.trangThai = 1")
//        Page<Object[]> listProductResponse(
//                @Param("ten") String ten,
//                Pageable pageable);
    ...
    @Query("SELECT gg.ten FROM SpGiamGia spgg JOIN spgg.giamGia gg WHERE gg.ten = :ten")
    boolean existsByGiamGia_ten(@Param("ten") String ten);
...
//    List<UUID> findProductIdsByDanhMucId(@Param("danhMucId") UUID danhMucId);
    ...
    @Modifying
    @Transactional
    @Query("UPDATE GiamGia gg SET gg.ten = :ten, gg.ngayBatDau = :ngayBatDau, gg.ngayKetThuc = :ngayKetThuc, gg.hinhThucGiam = :hinhThucGiam, gg.trangThai = :trangThai WHERE gg.id = :giamGiaId")
    ...
    // gg.ma,gg.ten, gg.ngayBatDau, gg.ngayKetThuc, gg.hinhThucGiam,
    // gg.trangThai, spgg.mucGiam, sp.ten, sp.giaBan, spgg.donGiaKhiGiam) " +
    // "FROM GiamGia gg " +
    // "JOIN gg.spGiamGiaList spgg " +
    // "JOIN spgg.sanPham sp " +
    // "WHERE gg.trangThai = 1 and gg.id = :id")
